...
public class XmlUtil implements Serializable {
   ...
   /**
    * Get one and only one child Element of passed node which has passed name.
    * @param node Node to examine
    * @param name String name to match
    * @return the matching element.
    * @throws Exception on error, or if 0 or more than 1 matching child element 
    * found.
    */
   public static Element getFirstLevelChildElementByName(Node node,
      String name) throws Exception{
      Element[] elements = getFirstLevelChildElementsByName(node, name);
      if (elements.length != 1) throw new Exception("getFirstLevelChildElementByName returned " + elements.length + " elements");
      return elements[0];
   }
